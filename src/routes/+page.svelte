<script lang="ts">
    ...
    $effect(()=>{
        ...
        // Adjust selectedSlide if it is out of bounds (e.g., after deletion)
        if (currentPresentation.slides.length > 0 && selectedSlide >= currentPresentation.slides.length) {
            selectedSlide = Math.max(0, currentPresentation.slides.length - 1);
        }
...
<Toolbar>
    ...
    <ToolbarGroup>
        <ToolbarButton iconSize=32 icon={Printer} caption="Print to PDF" clickEvent={()=>{goto(resolve('/present?print-pdf'))}}/>
        <ToolbarButton iconSize=32 icon={PresentationIcon} caption="Present" clickEvent={()=>{goto(resolve('/present'))}}/>
    </ToolbarGroup>
    <ToolbarGroup>
...
{#if currentPresentation.slides.length > 0}
    {#if currentPresentation.slides[selectedSlide]}
